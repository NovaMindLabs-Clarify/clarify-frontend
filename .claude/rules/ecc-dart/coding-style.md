...
> Источник: [ECC](https://github.com/affaan-m/ecc) `rules/dart/coding-style.md`. Раздел
> "Imports" ниже переписан — оригинал требовал исключительно `package:`-импорты, но
> **весь код этого проекта без исключения использует относительные импорты**
> (`import '../core/...'`). Следовать оригинальному правилу означало бы указывать
> направление, прямо противоречащее уже написанному коду.
...
## Imports (адаптировано под этот проект)

- Относительные импорты (`import '../core/...'`, `import 'clarify_surface.dart'`) — так уже
  написан весь код в этом репозитории, продолжаем в том же стиле, а не переходим на `package:`
- Порядок: `dart:` → внешние `package:` → относительные импорты внутри проекта

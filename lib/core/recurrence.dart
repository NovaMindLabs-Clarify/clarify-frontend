...
/// Точка отсчёта для следующего экземпляра.
///
/// По умолчанию — плановая дата ([dueDate]): «каждый понедельник» должно
/// оставаться каждым понедельником, даже если задачу отметили в среду.
///
/// Если у задачи включён повтор от выполнения ([fromCompletion]) — считаем от
/// момента, когда её реально отметили. Сценарий «поливать цветы каждые 3 дня
/// после того, как полил»: при отсчёте от плановой даты полив, сделанный с
/// недельным опозданием, назначил бы следующий на вчера.
///
/// [completedAt] может отсутствовать (отметка пришла без времени, старые данные)
/// — тогда честнее вернуться к плановой дате, чем молча подставить «сейчас».
DateTime recurrenceAnchor({
  required DateTime dueDate,
  required bool fromCompletion,
  DateTime? completedAt,
}) {
  if (!fromCompletion || completedAt == null) return dueDate;
  // Время внутри дня не нужно: даты задач хранятся с точностью до дня.

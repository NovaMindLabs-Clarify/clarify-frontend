...
void showManualAddDialog({
  ...
  required void Function(int days, int months, StateSetter setStateDialog, DateTime? currentDate) shiftDate,
                  ...
                  child: Column(
                    ...
                    children: [
                      Row(
                        ...
                        children: [
                          Text(isFromDuplicate ? "Дублирование".tr(currentLang) : "Новая задача".tr(currentLang), style: TextStyle(fontWeight: FontWeight.bold, fontSize: 20, color: textColor)),
                          IconButton(icon: Icon(LucideIcons.x, color: textMuted), onPressed: () { Navigator.pop(context); if (isDuplicating) onDuplicateHandled(); })
                        ]
                      ),
                      ...
                      Container(
                        ...
                        child: Column(
                          children: [
                            TextField(controller: tagsController, style: TextStyle(color: textColor), onChanged: (_) => setStateDialog(() {}), decoration: InputDecoration(labelText: "Теги (через запятую)".tr(currentLang), labelStyle: TextStyle(color: textMuted), border: InputBorder.none, isDense: true)),
                      ...
                      const SizedBox(height: 8),
                      Text("Чек-лист (Подзадачи):".tr(currentLang), style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16, color: textColor)),
                      const SizedBox(height: 8),
                        ...
                        Column(
                          children: tempSubtasks.asMap().entries.map((entry) {
                            ...
                            return Container(
                              ...
                              child: Row(
                                children: [
                                  Icon(LucideIcons.circle, size: 16, color: textMuted), const SizedBox(width: 8),
                                  Expanded(child: Text(subTitle, style: TextStyle(color: textColor))),
                                  IconButton(icon: Icon(LucideIcons.x, size: 16, color: t.danger), padding: EdgeInsets.zero, constraints: const BoxConstraints(), onPressed: () => setStateDialog(() => tempSubtasks.removeAt(idx)))
                                ],
                              ),
                            );
                          }).toList(),
                      ...
                      if (selectedMenu.startsWith('ws_')) ...[
                        ...
                        DropdownButtonFormField<String>(
                          ...
                          decoration: InputDecoration(
                            ...
                            prefixIcon: Icon(LucideIcons.user, color: textMuted, size: 20 * s),
                              ...
                              () async {
                                ...
                                if (newTaskId != null && tempSubtasks.isNotEmpty) {
                                  for (String subTitle in tempSubtasks) {
                                    await createTaskManually({"title": subTitle, "parent_id": newTaskId, "is_completed": false});
                                  }
                                }

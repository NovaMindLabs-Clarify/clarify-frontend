...
class _DesktopPlannerScreenState extends State<DesktopPlannerScreen> {
  ...
  Future<void> _fetchZenStatuses() async {
    try {
      ...
      print("Ошибка загрузки статусов Zen: $e");
    }
  }
    ...
    try {
      ...
      await Supabase.instance.client
          ...
          .update({
            ...
            'zen_until': newState
                ? DateTime.now()
                      .add(Duration(minutes: AppSettings.zenDurationMinutes))
                      .toIso8601String()
                : null,
          })
          .eq('user_id', user.id);
      ...
      if (mounted) {
        ClarifyToast.show(
          context,
          (newState
                  ? "Фокусирование включено! Уведомления заглушены"
                  : "Фокусирование выключено!")
              .tr(widget.currentLang),
          variant: ClarifyToastVariant.info,
        );
      }
    } catch (e) {
      print("Ошибка активации Zen: $e");
    }
  }
  ...
  List<Map<String, String>> chatMessages = [
    {
      'role': 'ai',
      'text':
          'Привет! Вставь текст, напиши руками или нажми на микрофон и надиктуй задачи голосом! 🔥',
    },
  ];
  ...
  ClarifyTokens get _tokens =>
      isDark ? ClarifyTokens.dark : ClarifyTokens.light;
  Color get glassColor =>
      _tokens.surface.withValues(alpha: isDark ? 0.55 : 0.7);
  ...
  Color get doneCardColor =>
      _tokens.surfaceSunken.withValues(alpha: isDark ? 0.7 : 0.85);
  ...
  Color get chatBubbleAi =>
      _tokens.surface2.withValues(alpha: isDark ? 0.6 : 0.8);
  Color get chatInput =>
      _tokens.surfaceSunken.withValues(alpha: isDark ? 0.7 : 0.85);
  ...
  void _initGlobalHotkeys() async {
    ...
    await hotKeyManager.register(
      searchKey,
      keyDownHandler: (hotKey) => showSearchDialog(
        context: context,
        currentLang: widget.currentLang,
        textColor: textColor,
        textMuted: textMuted,
    ...
    HotKey commandPaletteKey = HotKey(
    ...
    await hotKeyManager.register(
      commandPaletteKey,
      keyDownHandler: (hotKey) => _showCommandPalette(),
    );
  }
  ...
  String _getLocalKanbanStatus(dynamic taskId) =>
      _readKanbanStatusMap()[taskId.toString()] ?? 'todo';
  ...
  Future<void> _fetchWorkspaces() async {
    ...
    try {
      ...
      final List<int> workspaceIds = memberResponse
          .map<int>((m) => m['workspace_id'] as int)
          .toList();
  ...
  void _scheduleTaskAlarms(Map<String, dynamic> task) {
    ...
    setTimer(
      taskTime.subtract(const Duration(days: 3)),
      1,
      "Бро, скоро дедлайн!",
      "Задача '${task['title']}' через 3 дня.",
    );
    setTimer(
      taskTime.subtract(const Duration(days: 1)),
      2,
      "Завтра сдавать!",
      "Задача '${task['title']}' завтра в ${task['due_time']}.",
    );
    setTimer(
      taskTime.subtract(const Duration(hours: 1)),
      3,
      "Время пришло!",
    ...
    setTimer(
      ...
      4,
      "Дедлайн!",
      "Задача '${task['title']}' прямо сейчас!",
    );
  }
  ...
  void _initSpeech() async {
    ...
    try {
      ...
      if (_speechEnabled) {
        ...
        var ruLocale = systemLocales.firstWhere(
      ...
      print("Ошибка микрофона: $e".tr(widget.currentLang));
    }
  }
  ...
  void _toggleListening() async {
    ...
    if (_speechToText.isListening) {
      ...
      final currentText = _aiChatController.text;
      await _speechToText.listen(
        localeId: _currentLocaleId,
        onResult: (result) {
          setState(() {
            if (currentText.isEmpty) {
              _aiChatController.text = result.recognizedWords;
            } else {
              _aiChatController.text = "$currentText ${result.recognizedWords}";
            }
          });
        },
      );
  ...
  List<Map<String, dynamic>> get filteredTasks {
    ...
    if (activeTagFilter != null) {
      result = result
          .where((t) => parseTagsString(t['tags']).contains(activeTagFilter))
          .toList();
    }
    if (activePriorityFilter != null) {
      result = result
          .where((t) => t['priority'] == activePriorityFilter)
          .toList();
    }
    return result;
  }

  Map<String, int> _getSubtaskStats(dynamic parentId) {
    final subtasks = tasks.where((t) => t['parent_id'] == parentId).toList();
    if (subtasks.isEmpty) return {'total': 0, 'done': 0};
    return {
      'total': subtasks.length,
      'done': subtasks.where((t) => t['is_completed'] == true).length,
    };
  }
  ...
  Future<void> _fetchTasks() async {
    ...
    try {
      ...
      if (mounted) {
        ...
        ClarifyToast.show(
          context,
          "Нет сети. Работаем локально!".tr(widget.currentLang),
  ...
  Future<void> _sendTaskToAI(String text) async {
    ...
    setState(() {
      chatMessages.add({'role': 'user', 'text': text.trim()});
      isAiTyping = true;
    });
    ...
    try {
      ...
      setState(() {
        chatMessages.add({
          'role': 'ai',
          'text': 'Готово! Добавлено задач: $count. '.tr(widget.currentLang),
        });
        isAiTyping = false;
      });
      ...
      setState(() {
        chatMessages.add({
          'role': 'ai',
          'text': 'Ошибка: сервер вернул ${e.statusCode}',
        });
        isAiTyping = false;
      });
    } catch (e) {
      setState(() {
        chatMessages.add({
          'role': 'ai',
          'text': 'Ошибка связи с ИИ.'.tr(widget.currentLang),
        });
        isAiTyping = false;
      });
    }
    ...
    try {
      ...
      if (mounted) {
        ...
        final message = e is PostgrestException
            ...
            : "Не удалось сохранить задачу на сервере. Отправим, когда сеть восстановится."
                  .tr(widget.currentLang);
        ClarifyToast.show(
          context,
          message,
  ...
  Future<void> _updateTaskData(
    ...
    try {
      ...
      if (mounted) {
        ...
        final message = e is PostgrestException
            ? "Не удалось сохранить: ".tr(widget.currentLang) + e.message
            : "Нет сети. Изменение отправим, когда сеть восстановится.".tr(
                widget.currentLang,
              );
        ClarifyToast.show(
          context,
  ...
  Future<void> _spawnNextRecurringTask(Map<String, dynamic> task) async {
    DateTime? oldDate = _parseDate(task['due_date']);
  ...
  Future<void> _toggleTask(Map<String, dynamic> task) async {
    ...
    if (_pendingActionIds.contains(actionId))
      return; // Уже в процессе — игнорируем повторный тап

    final bool currentStatus = task['is_completed'] == true;
    final bool newStatus = !currentStatus;
    final currentCompletedAt = task['completed_at'];
    final String? newCompletedAt = newStatus
      ...
      builder: (context) {
        ...
        final content = Column(
          ...
          children: [
            ...
            Text(
              greeting.tr(widget.currentLang),
              style: TextStyle(
                color: textColor,
                fontSize: titleSize,
        ...
        return Center(
          child: Material(
            ...
            child: _buildGlassContainer(
              ...
              child: SizedBox(
                width: (MediaQuery.sizeOf(context).width - 96).clamp(
                  280.0,
                  420.0,
                ),
                child: content,
  ...
  void _checkBurnoutWarning(String dateStr) {
    if (_burnoutWarnedDates.contains(dateStr))
      return; // Уже предупреждали про этот день

    final dayTasks = tasks
        .where((t) => t['due_date'] == dateStr && t['parent_id'] == null)
        .toList();
            ...
            child: Material(
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  ...
                  border: Border.all(
                    color: _tokens.accent.withValues(alpha: 0.3),
                    width: 2,
                  ),
                  boxShadow: [
                    BoxShadow(
                      color: _tokens.accent.withValues(alpha: 0.1),
                      blurRadius: 20,
                      spreadRadius: 5,
                    ),
                  ],
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      "Эй, притормози! 🛑".tr(widget.currentLang),
                      style: TextStyle(
                        color: textColor,
                        fontSize: 24 * _s,
    ...
    try {
      ...
      if (mounted) {
        ...
        final message = e is PostgrestException
            ...
            : "Нет сети. Удаление отправим, когда сеть восстановится.".tr(
                widget.currentLang,
              );
        ClarifyToast.show(
          context,
  ...
  void shiftDate(
    ...
    DateTime baseDate = currentDate ?? DateTime.now();
    setStateDialog(
  ...
  void _showManualAddDialog({
    ...
    showManualAddDialog(
      ...
      onDuplicateHandled: () => setState(() {
        _isDuplicating = false;
        _taskToDuplicate = null;
      }),
    ...
    showTaskDetailsDialog(
      ...
      onDuplicate: (task) {
        ...
        if (isClarifyDialogMobile(context)) {
          _showManualAddDialog(
            sourceTaskForDuplicate: task,
            preselectedDate: DateTime.now(),
          );
        } else {
          setState(() {
            _taskToDuplicate = task;
            _isDuplicating = true;
          });
        }
      },
  ...
  void _handlePlusTap(DateTime date, int currentDayTaskCount) {
    if (currentDayTaskCount >= AppConfig.dailyTaskLimit) {
      ClarifyToast.show(
        context,
        "Лимит задач (100) исчерпан!".tr(widget.currentLang),
        variant: ClarifyToastVariant.danger,
      );
      return;
    }
    if (_isDuplicating && _taskToDuplicate != null) {
      _showManualAddDialog(
        ...
        sourceTaskForDuplicate: _taskToDuplicate,
      );
    } else {
      _showManualAddDialog(preselectedDate: date);
    }
  }
  ...
  void _handleTaskDropped({
    ...
    required int currentTargetTaskCount,
  }) async {
    final actionId = 'drop_${task['id']}';
    if (_pendingActionIds.contains(actionId))
      return; // Уже переносится — игнорируем повторный дроп

    await _guardedAction(actionId, () async {
      await _fetchTasks();
      _checkBurnoutWarning(targetDateStr);
      if (currentTargetTaskCount >= AppConfig.dailyTaskLimit) {
        ClarifyToast.show(
          context,
          "Лимит 100 задач!".tr(widget.currentLang),
          variant: ClarifyToastVariant.danger,
        );
        return;
      }
      if (HardwareKeyboard.instance.isControlPressed) {
        ...
        if (newTaskId != null && task['parent_id'] == null) {
          final subtasksToCopy = tasks
              .where((t) => t['parent_id'] == task['id'])
              .toList();
          for (var sub in subtasksToCopy) {
            await _createTaskManually({
              "title": sub['title'],
              "parent_id": newTaskId,
              "is_completed": false,
        ...
        if (mounted)
          ClarifyToast.show(
            context,
            "Скопировано!".tr(widget.currentLang),
            variant: ClarifyToastVariant.success,
          );
      } else {
        if (task['due_date'] != targetDateStr) {
          // Перенос вперёд перетаскиванием на невыполненной задаче тоже
          // считается переносом (см. AppConfig.rescheduleWarningCount) —
          // это как раз самый "невидимый" случай, задачу двигают одним
          // жестом не задумываясь.
  ...
  Map<String, dynamic> _parseSmartInput(String text) {
    ...
    final timeReg = RegExp(
      r'\b(?:в\s)?([0-1]?[0-9]|2[0-3]):([0-5][0-9])\b',
      caseSensitive: false,
    );
  ...
  Widget build(BuildContext context) {
    ...
    _s = (screenWidth / 1920).clamp(
      0.4,
      1.5,
    ); // Защищаем от сжатия в пыль и гигантизма
    ...
    if (screenWidth < 700) {
      return MobilePlannerShell(
        ...
        onAddTask: ({DateTime? preselectedDate}) =>
            _showManualAddDialog(preselectedDate: preselectedDate),
        createTaskManually: _createTaskManually,
    ...
    return AmbientBackground(
      ...
      child: Scaffold(
                ...
                child: Row(
                  children: [
                    ...
                    SidebarMenu(
                      ...
                      userAccountBlockCollapsed:
                          _buildUserAccountBlockCollapsed(),
                    ),
                    Expanded(
                      child: Column(
                                  ...
                                  children: [
                                    ...
                                    if (selectedMenu.startsWith('ws_')) ...[
                                        ...
                                        builder: (context) {
                                          ...
                                          return Row(
                                            children: members.map((m) {
                                              ...
                                              // Статус Дзена
                                              bool isZen =
                                                  zenStatuses[m['user_id']
                                                      .toString()] ==
                                                  true;

                                              // 🚀 РАДАР ВЫГОРАНИЯ: Считаем активные задачи на сегодня
                                              String todayStr = _formatDate(
                                                DateTime.now(),
                                              );
                                              int todayTasksCount = tasks
                                                  .where(
                                                    (t) =>
                                                        t['workspace_id'] ==
                                                            wsId &&
                                                        ...
                                                        t['is_completed'] !=
                                                            true &&
                                                        t['parent_id'] ==
                                                            null, // Считаем только главные задачи
                                                  )
                                                  .length;
                                                ...
                                                child: GestureDetector(
                                                  onTap: () => showUserProfileModal(
                                                    ...
                                                    onOpenConversation:
                                                        (
                                                          ...
                                                          _pendingChatPartnerId =
                                                              userId;
                                                          _pendingChatPartnerName =
                                                              name;
                                                          selectedMenu =
                                                              'Сообщения';
                                                        }),
                                                  ...
                                                  child: Tooltip(
                                                    ...
                                                    child: Opacity(
                                                      ...
                                                      child: Stack(
                                                        clipBehavior: Clip
                                                            .none, // Позволяем бейджику вылезать за края
                                                        alignment:
                                                            Alignment.center,
                                                        children: [
                                                          CircleAvatar(
                                      ...
                                      Padding(
                                        ...
                                        child: ClarifyIconButton(
                                          ...
                                          scale: _s,
                                          tooltip: "Покинуть/удалить команду"
                                              .tr(widget.currentLang),
                                          onPressed: () =>
                                              _showLeaveOrDeleteWorkspace(
                                                int.parse(
                                                  selectedMenu.substring(3),
                                                ),
                                              ),
                                    ...
                                    if (_pendingOpsCount > 0)
                                      Padding(
                                        ...
                                        child: Tooltip(
                                          message:
                                              "${'Несинхронизированных изменений: '.tr(widget.currentLang)}$_pendingOpsCount",
                                          child: Icon(
                                    ...
                                    ClarifyButton(
                                      variant: _isMyZenActive
                                          ? ClarifyButtonVariant.filled
                                          : ClarifyButtonVariant.outline,
                                      scale: _s,
                                      icon: LucideIcons.flower,
                                      label: _isMyZenActive
                                          ? "В фокусе".tr(widget.currentLang)
                                          : "Фокусирование".tr(
                                              widget.currentLang,
                                            ),
                                    ...
                                    ClarifyButton(
                                      variant: rightPanelState == 'ai'
                                          ? ClarifyButtonVariant.outline
                                          : ClarifyButtonVariant.filled,
                                      scale: _s,
                                      icon: rightPanelState == 'ai'
                                          ? LucideIcons.x
                                          : LucideIcons.sparkles,
                                      label: rightPanelState == 'ai'
                                          ? "Закрыть чат".tr(widget.currentLang)
                                          : "AI Ассистент".tr(
                                              widget.currentLang,
                                            ),
                                      onPressed: () => setState(
                                  ...
                                  if (activeTagFilter != null) ...[
                                    Container(
                                      ...
                                      decoration: BoxDecoration(
                                        ...
                                        border: Border.all(
                                          color: _tokens.accent.withValues(
                                            alpha: 0.4,
                                          ),
                                        ),
                                      ),
                                      child: Row(
                                        ...
                                        children: [
                                          ...
                                          SizedBox(width: 8 * _s),
                                          Text(
                                            "${'Тег: '.tr(widget.currentLang)}#$activeTagFilter",
                                            style: TextStyle(
                                              color: _tokens.accent,
                                              fontWeight: FontWeight.bold,
                                              fontSize: 15 * _s,
                                    ...
                                    Container(
                                      ...
                                      child: Row(
                                        ...
                                        children: [
                                          ...
                                          SizedBox(width: 8 * _s),
                                          Text(
                                            "${'Приоритет: '.tr(widget.currentLang)}${priorityFlagLabel(activePriorityFilter)}",
                                            style: TextStyle(
                                              color: _getPriorityColor(
                                                activePriorityFilter,
                                              ),
                                ...
                                child: Builder(
                                  builder: (context) {
                                    final taskCardBuilders = TaskCardBuilders(
                                      ...
                                      onPriorityTap: (priority) => setState(
                                    ...
                                    return MainContentArea(
                                      ...
                                      onToggleSortByPriority: () => setState(
                                        () => sortByPriority = !sortByPriority,
                                      ),
                                      pendingChatPartnerId:
                                          _pendingChatPartnerId,
                                      pendingChatPartnerName:
                                          _pendingChatPartnerName,
                                      onOpenDirectChat: (partnerId, name) =>
                                          setState(() {
                                      ...
                                      onReorderTasks:
                                          (oldIndex, newIndex, targetTasks) {
                                            setState(() {
                                              if (newIndex > oldIndex)
                                                newIndex -= 1;
                                              final item = targetTasks.removeAt(
                                                oldIndex,
                                              );
                                              targetTasks.insert(
                                                newIndex,
                                      ...
                                      buildListTaskCard:
                                          taskCardBuilders.buildListTaskCard,
                                      buildBoardTaskCardExpanded:
                                          taskCardBuilders
                                              .buildBoardTaskCardExpanded,
                                      buildCalendarTaskCard: taskCardBuilders
                                          .buildCalendarTaskCard,
                                      buildGlassContainer: _buildGlassContainer,
                                      getLocalKanbanStatus:
                                          _getLocalKanbanStatus,
                                      onSetLocalKanbanStatus:
                                          _setLocalKanbanStatus,
                    ...
                    ClipRect(
                      child: AnimatedContainer(
                        ...
                        width: rightPanelState != 'none' ? 360 * _s : 0,
                        child: OverflowBox(
        ...
        floatingActionButton: Padding(
          padding: EdgeInsets.only(
            right: rightPanelState != 'none' ? 360.0 * _s : 0.0,
            bottom: 24.0 * _s,
          ),
          ...
          child: FloatingActionButton.extended(
            ...
            elevation: 8,
            icon: Icon(LucideIcons.plus, size: 26 * _s),
            label: Text(
              "Создать задачу".tr(widget.currentLang),
              style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16 * _s),
            ),
          ),
        ),
      ),
  ...
  void _showAccountSettingsDialog() {
    showAccountSettingsDialog(
      ...
      isMobileContext:
          MediaQuery.of(context).size.width < ClarifyBreakpoints.mobile,
      buildGlassContainer: _buildGlassContainer,
    );
  }

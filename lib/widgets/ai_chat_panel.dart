...
class AiChatPanel extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        Container(padding: const EdgeInsets.fromLTRB(24, 32, 24, 24), decoration: BoxDecoration(border: Border(bottom: BorderSide(color: glassBorderColor))), child: Row(children: [const Icon(Icons.auto_awesome, color: Colors.blueAccent, size: 28), const SizedBox(width: 12), Text("AI Ассистент".tr(currentLang), style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: textColor))])),
        Expanded(
          child: ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              child: Text(msg['text']!.tr(currentLang), style: TextStyle(color: isAi ? textColor : Colors.white, fontSize: 15))));
            },
          ),
        ),
          ...
          child: TextField(
            ...
            decoration: InputDecoration(
              ...
              suffixIcon: Row(
                ...
                children: [
                  Container(margin: const EdgeInsets.symmetric(horizontal: 4), decoration: BoxDecoration(color: isListening ? Colors.red.withOpacity(0.15) : Colors.transparent, shape: BoxShape.circle), child: IconButton(icon: Icon(isListening ? Icons.mic : Icons.mic_none, color: isListening ? Colors.redAccent : Colors.blueAccent, size: 24), onPressed: onToggleListening)),
                  IconButton(icon: const Icon(Icons.send_rounded, color: Colors.blueAccent, size: 24), onPressed: () => onSend(controller.text)),
                ],
              ),
            ),
          ),
